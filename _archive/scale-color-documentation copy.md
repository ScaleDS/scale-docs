...
Scale uses a **two-layer color architecture** that separates raw color values from their meaningful application:

**Layer 1: Primitive Colors** are the foundational building blocks - specific hex values like `#3355ff` or `#1a1a1a`. These are theme-agnostic and never change. They represent the complete color palette with all its shades and variants.

**Layer 2: Semantic Colors** are meaningful tokens that describe **what a color does**, not what it looks like. A semantic token like `Background/Primary` maps to different primitive colors depending on the theme:
- In **light mode**: `Background/Primary` = `Neutral/900` (`#1a1a1a`)
- In **dark mode**: `Background/Primary` = `Neutral/50` (`#f2f2f2`)

This separation creates a powerful system where:
- **Designers** work with semantic tokens for consistency and theme adaptation
- **Developers** implement with semantic tokens for maintainability
- **Primitives** provide the raw materials for building the semantic system
- **Themes** can be changed without redesigning components
